...
import src.data.action as action
...
class PlanOnline:
    def __init__(self, plan_description):
        ...
        lines2 = [l for l in lines if l.find(";;(:metadata") >= 0]
        for l in lines2:
            azioni.append(l.split(":")[1].split("[")[0].replace("(","").replace(")","").strip())
        # if len(lines2) == 0:
        #   print(plan_description)
        init = lines2[0].split(";;(:metadata")[1].replace(")","")
        # if init.strip().startswith('<Action>'):
        #   print(plan_description)
        #Parse the xml line to get all the goals in ActGoals tags and the in Goal tags
        all_goals=[]
        for data in lines2:
            
            xml_data = data.split("(:metadata ", 1)[1].replace(")","").replace("(","")
            # Parse the XML string
            root = ET.fromstring(xml_data)
            # Extract the goals
            goals = [goal.text for goal in root.findall('Goal')]
            all_goals.append(goals)

        self.actions = [action.ActionOnline(a) for a in azioni]
        self.goals = all_goals

# plan=PlanOnline("/home/lorenzoserina/OnlineGR/prova_lpg/p000001_1.SOL_1.SOL")
